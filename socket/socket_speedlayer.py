...
def fitVar(p_lag, dataFrame):
    ...
    for select_y in x_list:
        if select_y != 'TimeStamp':
            ...
            print(select_y)
            # exec('predictions_{} = model_val.transform(dataFrame)'.format(str(select_y)))
    ...
    df_final = (df_RT_Temp.alias('dr').join(df_Nu_Temp.alias('dn'), on=df_RT_Temp['TimeStamp']== df_Nu_Temp['TimeStamp'],
                                            how='inner').selectExpr('dr.TimeStamp',
                                                                    'dr.RT_Temp as RT_Temp_Actual',
                                                                    'dr.prediction as RT_Temp_Predicted',
                                                                    'dn.Nu_Temp as Nu_Temp_Actual',
                                                                    'dn.prediction as Nu_Temp_Predicted')
                )
    df_final.show()
    df_final.write.saveAsTable(name='tsa.speed_predictions', format='hive', mode='append')
    # return lrModels

def pre_process(df, epoch_id):
    print('Starting....')
    ...
    if len(dfw.take(1)) != 0:
        ...
        # print('Total Models',len(l_lrmodels))
        # for l_models in range(0, len(l_models)):
        #     l_lrmodels[l_models].transform(dfw).show()
    ##### Checking dataM,.
...
query = lines.writeStream.trigger(processingTime='30 seconds').foreachBatch(pre_process).start()
