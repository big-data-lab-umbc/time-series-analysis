...
if __name__ == "__main__":
    ...
    data_file_name = '../source/testdata.csv'
    ...
    print('dataFrame_names',dataFrame_names)
    ...
    splits = assembler_for_lag.transform(dataFrame).randomSplit([0.7, 0.3])
    training_df = splits[0]
    test_df = splits[1]

    # training_df = assembler_for_lag.transform(dataFrame)
    print('train obs',training_df.count())
    print('test nobs',test_df.count())
    print('=== training frame ===')
    training_df.show()
    # fit the VAR models and put them into a list
    lrModels = []
    test_df = training_df.limit(17)
    test_df.show()
    for select_y in range(0, df_len_ori):
        ...
        print('==============VAR==============')
        exec('predictions_c{} = lrModel_c{}.transform(test_df)'.format(str(select_y), str(select_y)))
        exec('predictions_c{}.select("prediction","_c" + str({})).show()'.format(str(select_y),str(select_y)))
